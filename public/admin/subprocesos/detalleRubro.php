...
if (!isset($_POST['idRubro'])) {
    echo json_encode(['status' => 'error', 'message' => 'ID del rubro no fue proporcionado.']);
    exit;
}

$id = intval($_POST['idRubro']); // Recibe el ID de la solicitud

// Consulta para obtener los datos de la solicitud
$query = "SELECT * FROM rubros WHERE id = $id"; // Asegúrate de ajustar la tabla y campos según tu estructura
